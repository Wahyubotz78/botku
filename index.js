...
const { EditPhotoHandler } = require("./lib/edit_foto");
const { ChatAIHandler } = require("./lib/chat_ai");
...
async function startXeonBotInc() {
  ...
  // Batas Editabg dan Open AI⬇️
  XeonBotInc.ev.on("message", async (msg) => {
    ...
    try {
      ...
      for (let num of participants) {
        ...
        if (anu.action == "add") {
          ...
          let unicorndoc = {
            key: { fromMe: false, participant: "0@s.whatsapp.net", remoteJid: "916909137213-1604595598@g.us" },
            message: { orderMessage: { itemCount: 9999999, status: 200, thumbnail: XeonWlcm, surface: 200, message: `${metadata.subject}`, orderTitle: "xeon", sellerJid: "0@s.whatsapp.net" } },
...
└┬❖ 「 @${xeonName.split("@")[0]}  」
   ...
   │✑  *𝘑𝘰𝘪𝘯𝘦𝘥 :* 
   │✑ ${xtime} ${xdate}
   │✑ *OPEN JASA INSTALL SERVER PANEL PTERODACTYL*
   |
   │        - VPS DARI KALIAN (belum stock Vps) 
   │        - FREE SUBDOMAIN MY.ID
   |        - FREE INSTALL WINGS
   |        - FREE EGGS
   |        - FREE INSTALL PANEL COIN (JIKA MAU)
   |        
   │✑ *HARGA 10.000* 
   │✑ _HUB ADMIN_ : wa.me/6285808654304 (Fast Respon)
   │✑ indo-mypanel.my.id
   └───────────────┈ ⳹`;
          let buttonMessage = {
          ...
          XeonBotInc.sendMessage(anu.id, { quoted: unicorndoc });
          ...
          let unicorndoc = {
            key: { fromMe: false, participant: "0@s.whatsapp.net", remoteJid: "916909137213-1604595598@g.us" },
            message: { orderMessage: { itemCount: 9999999, status: 200, thumbnail: xeonbuffer, surface: 200, message: `${metadata.subject}`, orderTitle: "xeon", sellerJid: "0@s.whatsapp.net" } },
            contextInfo: { forwardingScore: 999, isForwarded: true },
            sendEphemeral: true,
          };
          xeonbody = `┌─❖
...
└┬❖ 「 @${xeonName.split("@")[0]}  」
  ...
  XeonBotInc.sendTextWithMentions = async (jid, text, quoted, options = {}) =>
    XeonBotInc.sendMessage(jid, { text: text, contextInfo: { mentionedJid: [...text.matchAll(/@(\d{0,16})/g)].map((v) => v[1] + "@s.whatsapp.net") }, ...options }, { quoted });
